...
class UpdateEmail extends GetxController {
  static UpdateEmail get instance => Get.find();

  final email = TextEditingController();
  final userController = UserController.instance;
  final userRepository = Get.put(UserRepository());
  GlobalKey<FormState> updateemail = GlobalKey<FormState>();
  ...
  Future<void> updateUserEmail() async {
    try {
      ...
      if (!updateemail.currentState!.validate()) {
        TFullScreenLoader.stopLoading();
        return;
      }
      //update user's first name and last name in firebase
      Map<String, dynamic> mail = {
        'firstName': email.text.trim(),
        
      };
      await userRepository.updateSingleField(mail);

      //Show success message
      TLoaders.successSnackbar(
          message: 'Your email has been updated', title: 'Congrats!');
